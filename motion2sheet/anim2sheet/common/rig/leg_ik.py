"""Humanoid leg IK convention and knee-guide authority diagnostics."""
...
def rest_bone_debug(arm, name: str) -> dict:
    ...
    return {"name": name, "headLocal": vec(Vector(bone.head_local)), "tailLocal": vec(Vector(bone.tail_local)),
            "xAxisArmature": vec(Vector(basis.col[0]).normalized()), "yAxisArmature": vec(Vector(basis.col[1]).normalized()),
...
def constraint_debug(arm, cfg: dict) -> dict:
    ...
    return {"bone": cfg["shin"], "name": constraint.name, "chainCount": int(constraint.chain_count),
...
def frame_leg_debug(arm, frame: int, side: str, cfg: dict) -> dict:
    ...
    return {"hip": vec(hip), "knee": vec(knee), "ankle": vec(ankle), "kneeGuide": vec(guide),
            "evaluatedBendDirection": vec(knee_dir), "guideBendDirection": vec(guide_dir),
            ...
            "alignmentCos": round(alignment, 6), "match": bool(alignment > 0.0)}
