...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--visible-meshes", default="")
    return parser.parse_args(argv())


def visible_mesh_names(args: argparse.Namespace) -> set[str]:
    return {value.strip() for value in args.visible_meshes.split(",") if value.strip()}
...
def prepare_armature(arm, visible_meshes: set[str]):
...
def set_visible_meshes(names: set[str], visible: bool):
    for name in names:
...
def render_default_rig(arm, root, window, area, region, visible_meshes):
    ...
    set_visible_meshes(visible_meshes, False)
    ...
    set_visible_meshes(visible_meshes, True)
    scene.render.resolution_x, scene.render.resolution_y, scene.render.resolution_percentage = old_resolution
    scene.frame_set(old_frame)
    bpy.context.view_layer.update()


def render_animation(arm, output, window, area, region, visible_meshes, frames=None):
    scene = bpy.context.scene
...
def main() -> int:
    ...
    visible_meshes = visible_mesh_names(args)
    arm = find_armature()
    prepare_armature(arm, visible_meshes)
    ...
    if not args.skip_rig_docs:
        render_default_rig(arm, root, window, area, region, visible_meshes)
    render_animation(arm, output, window, area, region, visible_meshes, frames=frames)
