...
def run_blender(spec_path: Path, output: Path, blender_name: str) -> None:
    ...
    script = package_root() / "blender" / "native_generate_vfx_v7.py"
    subprocess.run([
...
def build(args) -> int:
    ...
    metadata = {
        "tool": "vfx2sheet", "version": 7, "template": spec.template, "variant": spec.variant,
        ...
        "renderer": "blender-native-editable-source-v7",
