...
def run_blender(spec_path: Path, output: Path, blender_name: str) -> None:
    ...
    script = package_root() / "blender" / "native_generate_vfx_v8.py"
    subprocess.run([
...
def build(args) -> int:
    ...
    metadata = {
        "tool": "vfx2sheet", "version": 8, "template": spec.template, "variant": spec.variant,
        ...
        "renderer": "blender-native-editable-source-v8",
