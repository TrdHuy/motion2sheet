...
from .math3d import clean, clean_vec, dot, length, mul3v, rotation4, unit
...
def _bone_y(rotation):
    return rotation[0][1], rotation[1][1], rotation[2][1]


def _source_bone_direction(retargeted: dict, frame: dict, mapping: dict, target_name: str, *, rest: bool):
    ...
    return mul3v(mapping["sourceToTargetAxes"], _bone_y(rotation4(matrix)))


def _target_bone_direction(retargeted: dict, frame: dict, target_name: str, *, rest: bool):
...
def source_retarget_fidelity(retargeted: dict, mapping: dict) -> dict:
    ...
    for index, frame in enumerate(retargeted["frames"], start=1):
        ...
        left_right_ok = float(target_points["leftShoulder"][0]) < float(target_points["rightShoulder"][0]) and float(target_points["leftHip"][0]) < float(target_points["rightHip"][0])
        if not left_right_ok:
            left_right_failures.append({"frame": index, "sourceFrame": int(frame["sourceFrame"])})

        rows.append({"frame": index, "sourceFrame": int(frame["sourceFrame"]), "segmentDirectionErrorsDegrees": segment_errors, "bendErrors": bend_errors, "rootDirectionErrorDegrees": clean(root_error, 9), "ordering": frame_ordering})

    max_arm, max_leg = max_arm_direction[0], max_leg_direction[0]
    max_elbow_value, max_knee_value = max_elbow[0], max_knee[0]
    max_root, max_vertical_value = max_root_direction[0], max_vertical[0]
    passed = (
    ...
    return {
        ...
        "perSegment": per_segment, "perJoint": per_joint,
        "orderingMismatches": ordering_mismatches, "leftRightFailures": left_right_failures, "frames": rows,
    }
