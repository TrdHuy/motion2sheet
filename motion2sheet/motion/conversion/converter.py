...
from .diagnostics import conversion_fidelity, representation_limitations, target_pose_rows
...
def _write_json(path: Path, data: dict) -> None:
    ...
    path.write_text(
        json.dumps(data, ensure_ascii=False, indent=2, sort_keys=True, allow_nan=False, separators=(",", ": ")) + "\n",
        encoding="utf-8",
    )
...
def _validate_character_target(character_path: Path, target_rig_path: Path, target_rig: dict) -> dict:
    ...
    if character_rig != target_rig:
        raise ValueError(
            "character profile rig does not exactly match requested target rig: "
            f"character={resolution['rigPath']} target={target_rig_path}"
        )
...
def convert_animation_profile(*, source_rig_path: Path, source_animation_path: Path, target_rig_path: Path, mapping_path: Path, character_profile_path: Path, output: Path) -> dict:
    ...
    motion = {
        "schema": "anim2sheet.motion",
        "version": 2,
        "id": motion_id,
        "rigProfile": _relative(output, target_rig_path),
        "fps": float(source_animation["fps"]),
        "frameCount": int(source_animation["frameCount"]),
        "space": str(target_rig["motionContract"]["space"]),
        "frames": normalized["frames"],
        "provenance": {
            "sourceContract": {"rigId": str(source_rig["id"]), "animationId": str(source_animation["id"])},
            "mappingProfile": str(mapping["id"]),
            "conversion": "source-authority-pose-retarget-then-motion-contract-normalization",
        },
    }
    animation_profile = {
        "schema": "anim2sheet.animation",
        "version": 2,
        "id": motion_id,
        "action": action,
        "motionProfile": "motion.json",
        "defaultCharacterProfile": _relative(output, character_profile_path),
        "loop": False,
    ...
    report = {
        "schema": "motion2sheet.animation-profile-conversion",
        "version": 1,
        "sourceMotionFileRequired": False,
        "architecture": {
            "sourceAuthority": "Contract B animation.frames[].bones + rig.editGeometry",
            "retarget": "evaluate source pose -> target rest-relative world orientation transfer",
            "normalization": "derive channels declared by target rig motionContract/solvers",
            "contractA": "Anim2Sheet Profile Contract v2",
        },
        "source": {
            "rigId": str(source_rig["id"]),
            "animationId": str(source_animation["id"]),
        ...
        "retarget": {
            "sourceStature": retargeted["sourceStature"],
            "targetStature": retargeted["targetStature"],
            "rootMotionScale": retargeted["rootMotionScale"],
            "mappingStrategy": "explicit-one-to-one-semantic-anchor-map-no-fuzzy-fallback",
            "mappedBones": dict(sorted(mapping["targetToSource"].items())),
        },
        "normalization": {"targetMotionContract": target_rig["motionContract"], "frameDiagnostics": normalized["diagnostics"]},
        "fidelity": fidelity,
        "targetPoseFrames": target_pose_rows(retargeted),
    ...
    if not fidelity["pass"]:
        raise ValueError(
            "Contract A expressiveness gate failed: "
            f"max={fidelity['maxErrorMeters']}m tolerance={fidelity['toleranceMeters']}m "
            f"at F{fidelity['worstFrame']} {fidelity['worstSemantic']}; see {output / 'conversion.json'}"
        )
    return report
