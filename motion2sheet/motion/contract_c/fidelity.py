...
def _matrix_rotation(matrix: list[list[float]]) -> list[float]:
    ...
    if trace > 0.0:
        ...
        quaternion = [
            0.25 * size,
            (rotation[2][1] - rotation[1][2]) / size,
            (rotation[0][2] - rotation[2][0]) / size,
            (rotation[1][0] - rotation[0][1]) / size,
        ]
        ...
        quaternion = [
            (rotation[2][1] - rotation[1][2]) / size,
            0.25 * size,
            (rotation[0][1] + rotation[1][0]) / size,
            (rotation[0][2] + rotation[2][0]) / size,
        ]
        ...
        quaternion = [
            ...
            (rotation[0][1] + rotation[1][0]) / size,
            0.25 * size,
            (rotation[1][2] + rotation[2][1]) / size,
        ]
    else:
...
def _world_matrices(rig: dict[str, Any], pose: dict[str, dict[str, Any]]) -> dict[str, list[list[float]]]:
    ...
    def resolve(name: str) -> list[list[float]]:
        ...
        result[name] = (
            _matrix_multiply(armature, local)
            if bone["parent"] is None
            else _matrix_multiply(resolve(bone["parent"]), local)
        )
        return result[name]
...
def _validate_source_hierarchy_mode(rig: dict[str, Any], joints: dict[str, str]) -> None:
    ...
    for name in sorted(required):
        ...
        if (
            not properties["useInheritRotation"]
            or not properties["useLocalLocation"]
            or properties["inheritScale"] != SUPPORTED_INHERIT_SCALE
        ):
...
def _quaternion_report(animation: dict[str, Any]) -> dict[str, Any]:
    tracks = {
        "Root": animation["root"]["rotations"],
        "Hips": animation["hips"]["rotations"],
        **{semantic: animation["joints"][semantic]["rotations"] for semantic in animation["joints"]},
    }
    ...
    for semantic, track in tracks.items():
        ...
        for sample, raw in enumerate(track):
            ...
            if previous is None:
                ...
                sign_pass = sign_pass and (
                    dot >= -QUATERNION_CONTINUITY_TOLERANCE
                    and (abs(dot) > QUATERNION_CONTINUITY_TOLERANCE or first >= 0.0)
                )
            previous = raw
    return {
        "pass": maximum_norm_error <= QUATERNION_NORM_TOLERANCE and sign_pass,
        "maxNormError": maximum_norm_error,
        "worstNorm": worst_norm,
        "minimumAdjacentDot": minimum_adjacent_dot,
        "worstContinuity": worst_continuity,
        "signContinuityPass": sign_pass,
        "tolerances": {
            "norm": QUATERNION_NORM_TOLERANCE,
            "continuityDot": QUATERNION_CONTINUITY_TOLERANCE,
        },
    }


def compare_source_to_contract_c(
    source_rig: dict[str, Any],
    source_animation: dict[str, Any],
    mapping: dict[str, Any],
    contract_c: dict[str, Any],
) -> dict[str, Any]:
    failures: list[str] = []
    try:
        animation = validate_animation(copy.deepcopy(contract_c))
        ...
        return {
            ...
            "schemaValidation": {"pass": False, "error": str(exc)},
            "failures": [f"Contract C schema validation failed: {exc}"],
        }
    ...
    source_offsets = [
        [(position[index] - hips_rest_position[index]) / leg_length for index in range(3)]
        for position in hips_positions
    ]
    planar_end_to_end = [
        source_offsets[-1][0] - source_offsets[0][0],
        source_offsets[-1][1] - source_offsets[0][1],
        0.0,
    ]
    ...
    for sample, (source_frame, world, hips_rotation, source_offset) in enumerate(
        zip(
            frames[:comparison_count],
            source_world[:comparison_count],
            hips_rotations[:comparison_count],
            source_offsets[:comparison_count],
        )
    ):
        ...
        expected_root_rotation = _yaw_twist(
            _quaternion_multiply(hips_rotation, _quaternion_inverse(first_hips_rotation))
        )
        ...
        expected_hips = _quaternion_rotate(
            _quaternion_inverse(expected_root_rotation),
            [source_offset[index] - stripped[index] for index in range(3)],
        )
        actual_hips = (
            animation["hips"]["translations"][sample]
            if animation["hips"]["translations"]
            else [0.0, 0.0, 0.0]
        )
        ...
        for semantic, bone_name in joints.items():
            expected_rotation = _quaternion_normalize(
                _quaternion_multiply(
                    _quaternion_multiply(
                        _quaternion_inverse(expected_root_rotation),
                        _matrix_rotation(world[bone_name]),
                    ),
                    _quaternion_inverse(rest_rotations[semantic]),
                )
            )
            actual_rotation = (
                animation["hips"]["rotations"][sample]
                if semantic == "Hips"
                else animation["joints"][semantic]["rotations"][sample]
            )
    ...
    if not root_pass:
        failures.append("Contract C Root translation is not in-place")
    ...
    for suffix in ("Shoulder", "UpperArm", "LowerArm", "Hand", "UpperLeg", "LowerLeg", "Foot", "Toe"):
        ...
        pair_pass = (
            joints[left_semantic] != joints[right_semantic]
            and left_x > right_x + 1e-6
            and semantic_maxima[left_semantic] <= ROTATION_TOLERANCE_DEGREES
            and semantic_maxima[right_semantic] <= ROTATION_TOLERANCE_DEGREES
        )
        left_right_pairs.append({
            "semanticPair": suffix,
            "leftBone": joints[left_semantic],
            "rightBone": joints[right_semantic],
            "leftX": left_x,
            "rightX": right_x,
            "pass": pair_pass,
        })
    ...
    return {
        ...
        "source": {
            "rigId": source_rig["id"],
            "animationId": source_animation["id"],
            "frameCount": source_animation["frameCount"],
            "fps": source_animation["fps"],
            "meanLegLengthSceneUnits": leg_length,
        },
        "contractC": {
        ...
        "tolerances": {
            "fps": FPS_TOLERANCE,
            "rotationDegrees": ROTATION_TOLERANCE_DEGREES,
            "hipsTranslationMeanLegLength": HIPS_TRANSLATION_TOLERANCE,
            "rootTranslationMeanLegLength": ROOT_TRANSLATION_TOLERANCE,
        },
        "timing": {"pass": frame_count_pass and fps_error <= FPS_TOLERANCE, "fpsError": fps_error},
        "maxErrors": {
            "rootRotationDegrees": max_root_rotation_error,
            "semanticRotationDegrees": max_semantic_rotation_error,
            "hipsTranslationMeanLegLength": max_hips_translation_error,
            "rootTranslationMeanLegLength": root_max_abs,
        },
        "worstRootRotation": worst_root_rotation,
        "worstSemantic": worst_semantic_rotation,
        "worstHipsTranslation": worst_hips_translation,
        "semanticRotationMaximaDegrees": semantic_maxima,
        "rootInvariant": {
            "pass": root_pass,
            "tolerance": ROOT_TRANSLATION_TOLERANCE,
            "maxAbsComponent": root_max_abs,
            "maxMagnitude": root_max_magnitude,
            "worstFrame": root_worst,
        },
        "locomotionStripping": {
            "pass": locomotion_pass,
            "policy": "linear-endpoint-planar-detrend-v1",
            "sourcePlanarEndToEnd": planar_end_to_end,
            "sourcePlanarDisplacement": planar_displacement,
            "sourceHadPlanarLocomotion": planar_displacement > LOCOMOTION_DETECTION_TOLERANCE,
            "detectionTolerance": LOCOMOTION_DETECTION_TOLERANCE,
            "strippedPlanarEndToEnd": planar_end_to_end,
            "expectedHipsVerticalRange": expected_vertical_range,
            "actualHipsVerticalRange": actual_vertical_range,
            "verticalRangeError": abs(expected_vertical_range - actual_vertical_range),
        },
        "leftRightVerification": {
            "pass": left_right_pass,
            "rightAxis": "+X",
            "pairs": left_right_pairs,
        },
