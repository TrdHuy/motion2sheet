...
def _run_blender(script: str, blender: str, arguments: list[str]) -> None:
    ...
    subprocess.run(
        [_blender(blender), "--background", "--factory-startup", "--python-exit-code", "1", "--python", str(path), "--", *arguments],
        check=True,
    )
...
def parse_samples(value: str, frame_count: int) -> list[int]:
    ...
    if not selected:
        raise ValueError("--frames selected no Contract C samples")
    if len(set(selected)) != len(selected):
        raise ValueError("--frames contains duplicate Contract C samples")
...
def export_contract_c_animation(
    *,
    ...
    for path in (source_rig_path, source_animation_path, mapping_path):
        if not path.is_file():
            raise ValueError(f"export-contract-c-animation input does not exist: {path}")
    ...
    request = {
        "sourceRigPath": str(source_rig_path),
        "sourceAnimationPath": str(source_animation_path),
        "mappingPath": str(mapping_path),
        "animationId": animation_id,
        "loop": loop,
        "animationOutput": str(output / "animation.json"),
        "diagnosticsOutput": str(output / "diagnostics" / "export_blender.json"),
    }
    ...
    report = {
        ...
        "version": 1,
        "contractCSchema": animation["schema"],
        "animationId": animation["id"],
        "canonicalSkeleton": animation["canonicalSkeleton"],
        "fps": animation["fps"],
        "frameCount": animation["frameCount"],
        "loop": animation["loop"],
        "animationSha256": sha256(output / "animation.json"),
        "sourceAuthorities": {
            "rigSha256": sha256(source_rig_path),
            "animationSha256": sha256(source_animation_path),
...
def render_contract_c_animation(
    *,
    model_path: Path,
    ...
    paths = [model_path, character_rig_path, skin_path, mapping_path, animation_path, camera_profile_path]
    paths = [path.resolve() for path in paths]
    model_path, character_rig_path, skin_path, mapping_path, animation_path, camera_profile_path = paths
    for path in paths:
        if not path.is_file():
            raise ValueError(f"render-contract-c-animation input does not exist: {path}")
    rig = validate_rig_document(read_json(character_rig_path))
    skin = validate_skin_document(read_json(skin_path), rig)
    mapping = validate_character_mapping(read_mapping(mapping_path), rig)
    ...
    request = {
        ...
        "animationSha256": before_sha,
        "camera": camera,
        "selectedSamples": selected,
        "selectedFrames": [sample + 1 for sample in selected],
        "canvas": list(canvas),
    ...
    if before_sha != after_sha:
        raise RuntimeError("Contract C animation was mutated during playback")
    ...
    if missing:
        raise RuntimeError(f"Contract C render missing frames: {missing[:4]}")
    ...
    diagnostics = {
        name: json.loads((output / "diagnostics" / f"{name}.json").read_text(encoding="utf-8"))
        for name in ("model_identity", "skin_reconstruction", "semantic_mapping", "retarget", "playback", "root_motion", "contact")
    }
    if not diagnostics["skin_reconstruction"]["pass"] or not diagnostics["playback"]["pass"]:
        raise RuntimeError("Contract C runtime fidelity failed; see diagnostics")
    report = {
        "schema": "motion2sheet.contract-c.render",
        "version": 1,
        "contractCSchema": animation["schema"],
        "animationId": animation["id"],
        "canonicalSkeleton": animation["canonicalSkeleton"],
        ...
        "sourceFbxRequired": False,
        "sourceRigRequired": False,
        "runtimeTransformsAreAnimationAuthority": False,
        "skinStatistics": skin_statistics(skin, rig),
        "renderedSamples": selected,
        "renderSamples": render_samples,
        "layout": {"cellSize": list(canvas), "sheetColumns": sheet_columns, **layout},
        "gifTiming": gif_timing,
        "semanticMapping": diagnostics["semantic_mapping"],
        "rootMotion": diagnostics["root_motion"],
        "retarget": diagnostics["retarget"],
        "playback": diagnostics["playback"],
        "contact": diagnostics["contact"],
        "outputs": {"poseSheet": "pose_sheet.png", "previewGif": "preview.gif" if gif else None, "diagnostics": "diagnostics/"},
    }
