...
def main():
    ...
    source=build_armature('ContractBReference',source_bones(rig),rig['armatureObject']['transform'],True); char=build_armature('CharacterRig',request['character']['rig']['bones'],request['character']['rig']['objectTransform'],False); build_body(request['character'],char); build_equipment(request['character'],char)
    ...
    if not report['pass']: raise RuntimeError(f'direct Contract B playback fidelity failed: {report}')
