...
def test_component_change_runs_only_dependent_targets():
    _,targets=resolve("motion2sheet/motion/normalize/core.py"); assert "motion-normalize-unit" in targets; assert "motion-synthetic-fbx" in targets; assert "motion-humanoid-motion-e2e" not in targets; assert "vfx-unit" not in targets; assert not any(name.endswith("-e2e") and name.startswith("anim-") for name in targets)
def test_vfx_effect_change_does_not_run_motion_or_anim():
    manifest=load_manifest(); components,targets=resolve("motion2sheet/vfx2sheet/effects/splash/config.py",manifest=manifest); assert "vfx-splash" in components; assert "vfx-unit" in targets; assert "vfx-splash-e2e" in targets; assert "motion-humanoid-motion-e2e" not in targets; assert not anim_e2e_targets(manifest,targets)
...
def test_humanoid_motion_change_routes_its_unit_and_motion_cli_dependents_only():
    manifest=load_manifest(); components,targets=resolve("motion2sheet/motion/humanoid_motion/schema.py",manifest=manifest)
    assert {"motion-humanoid-motion","motion-cli"}.issubset(components)
    assert "motion-humanoid-motion-unit" in targets
    assert "motion-humanoid-motion-e2e" in targets
...
def test_humanoid_motion_profile_change_uses_humanoid_motion_component():
    manifest=load_manifest(); components,targets=resolve("profiles/humanoid_motion/mixamo_humanoid_v1.json",manifest=manifest)
    assert "motion-humanoid-motion" in components and "motion-humanoid-motion-unit" in targets and "motion-humanoid-motion-e2e" in targets
def test_humanoid_motion_test_change_selects_only_humanoid_motion_unit():
    manifest=load_manifest(); components,targets=resolve("tests/motion/humanoid_motion/test_humanoid_motion.py",manifest=manifest)
    assert components==set(); assert targets=={"motion-humanoid-motion-unit"}
def test_humanoid_motion_e2e_runner_change_selects_unit_and_e2e():
    manifest=load_manifest(); components,targets=resolve("tests/motion/humanoid_motion/run_e2e.sh",manifest=manifest)
    assert components==set(); assert targets=={"motion-humanoid-motion-unit","motion-humanoid-motion-e2e"}
def test_motion_cli_change_selects_humanoid_motion_e2e():
