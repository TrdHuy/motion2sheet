...
def test_spec_applies_known_numeric_and_color_overrides():
    spec = VfxSpec.create(
        template="slash", variant="lightning", seed=1234,
        overrides=[
...
def test_hierarchical_lightning_width_bounds_are_validated():
    with pytest.raises(ValueError, match="major_width_min"):
        VfxSpec.create(template="slash", variant="lightning", overrides=["lightning.major_width_min=6", "lightning.major_width_max=4"])
    with pytest.raises(ValueError, match="tip_width"):
        VfxSpec.create(template="slash", variant="lightning", overrides=["lightning.tip_width=6", "lightning.major_width_max=5"])
...
def test_hierarchical_count_override_rejects_fraction():
    ...
    with pytest.raises(ValueError, match="must be an integer"):
        parse_set("core.streak_count=2.5")


def test_out_of_range_override_is_rejected_immediately():
    with pytest.raises(ValueError, match="must be in range"):
        VfxSpec.create(template="slash", variant="lightning", overrides=["radius=99"])
